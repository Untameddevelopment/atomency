...
class IsotopeSystem {
    ...
    calculateIsotopicMass(protons, neutrons) {
        const protonMass = 1.007276;    // amu
        const neutronMass = 1.008665;   // amu
        const electronMass = 0.000549;  // amu

        const mass = (protons * protonMass) + (neutrons * neutronMass) - (protons * electronMass);

        // Apply mass defect correction (binding energy effect)
        // Heavier nuclei have mass defect
        const massDefect = 0.9 - (0.001 * Math.sqrt(protons + neutrons));
        return Math.max(0, mass * (1 - massDefect / 1000));
    }
    ...
    calculateMolecularMass(atoms, bonds) {
        ...
        atoms.forEach(atom => {
            ...
            const stdWeight = element.atomicMass || element.atomic_mass || 0;
            totalMass += stdWeight;
        });
